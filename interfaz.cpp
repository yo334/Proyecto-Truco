...
void Interfaz::mostrarTablero(const string& nombre_jugador, int pts_jugador, const string& nombre_ia, int pts_ia, int objetivo) {
    ...
    cout << "+----------------------------------+\n";
    cout << "|  PUNTAJE                         |\n";
    cout << "|  " << left << setw(10) << nombre_jugador << ": " << generarBarra(pts_jugador) << " " << setw(2) << pts_jugador << "/" << objetivo << "  |\n";
    cout << "|  " << left << setw(10) << nombre_ia << ": " << generarBarra(pts_ia) << " " << setw(2) << pts_ia << "/" << objetivo << "  |\n";
    cout << "+----------------------------------+\n";
}

void Interfaz::mostrarEstadoMano(const vector<Carta>& mano_jugador, int cartas_ia, const vector<string>& resultados_rondas) {
    cout << "\n--- Estado de la Mano ---\n";
    cout << "Cartas IA:\n";
...
void Interfaz::mostrarBanner() {
    ...
    cout << "       ¡Preparate para el mejor Truco en consola!\n\n";
}
 ...
 * Se diseñó agrupando funciones de entrada/salida como métodos estáticos para separar la lógica de presentación de la del juego, lo que se alinea con el principio de responsabilidad única (Single Responsibility Principle).
 * Conceptos de C++: 
 * - Uso de directivas del preprocesador `#ifdef _WIN32` para compilación condicional según el sistema operativo, permitiendo limpiar la consola en Windows o Linux apropiadamente.
 * - `std::setw` y `std::left` de la librería `<iomanip>` para formatear el texto del tablero y dejarlo prolijo.
 */
