...
Carta Mazo::repartir() {
    if (cartas.empty()) {
        throw std::out_of_range("¡Pará un poco fiera! El mazo está vacío, no se puede repartir.");
    }
...
void Mazo::reiniciar() {
    ...
    std::vector<std::string> palos = {"Espada", "Basto", "Oro", "Copa"};
    std::vector<int> numeros = {1, 2, 3, 4, 5, 6, 7, 10, 11, 12};
...
int Mazo::cantidadRestante() const {
...
- Concepto C++: Uso dinámico de contenedores de la Standard Template Library 
  (STL). Usamos `std::vector` que es un arreglo dinámico, re piola porque 
  crece y se achica solo (con `push_back` y `pop_back`).
- Otro tema clave es la mezcla: En C++ moderno (desde C++11) usamos 
  `<random>` con `std::mt19937` y `std::shuffle` en vez del viejo `rand()`. 
  Esto garantiza una distribución probabilística muchísimo mejor (más "aleatoria" 
  de verdad), para que no te caguen en el truco repartiendo siempre lo mismo.
=============================================================================
*/
